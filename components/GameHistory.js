...
const GameHistory = () => {
  ...
  const storeData = async value => {
    try {
      ...
      await AsyncStorage.setItem('@GameHistory', jsonValue);
  ...
  const debugView = (
    <View>
      ...
      <Text>goal is ({goal})</Text>
      <Text>result is ({result})</Text>
      <Text>GameHistory is {JSON.stringify(GameHistory)}</Text>
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={{fontSize: 12}}>Enter the game history below</Text>
      ...
      <View style={{flexDirection: 'row', justifyContent: 'space-around'}}>
        <Button
          ...
          color="blue"
          onPress={() => {
            const newGameHistory = gameHistory.concat({
              dateTime: dateTime,
              goal: goal,
              result: result,
              completed: new Date(),
      ...
      >
        <Text
          style={{fontSize: 20, color: 'green', backgroundColor: 'lightgray'}}
        >
          History of Games
        </Text>
      </View>
